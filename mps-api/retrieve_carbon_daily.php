...
if (!empty($_POST) && isset($_POST['token'])) {
    ...
    if (!($items_statement = $mysqli->prepare("SELECT date_added AS date, SUM(carbon) AS total_carbon FROM mps_user_items WHERE user_id = ? GROUP BY date_added ORDER BY date_added ASC"))) {
