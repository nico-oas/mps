...
if (!empty($_POST) && isset($_POST['token'])) {
    ...
    if (!($items_statement = $mysqli->prepare("SELECT item_name AS name, item_category AS category, carbon, date_added AS add_date FROM mps_user_items WHERE user_id = ? ORDER BY date_added ASC"))) {
