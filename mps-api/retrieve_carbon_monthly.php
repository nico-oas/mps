...
if (!empty($_POST) && isset($_POST['token'])) {
    ...
    if (!($carbon_statement = $mysqli->prepare("SELECT SUM(carbon) AS total_carbon, CONCAT(YEAR(date_added), '-', MONTH(date_added)) AS date FROM mps_user_items WHERE user_id = ? GROUP BY YEAR(date_added), MONTH(date_added) ORDER BY YEAR(date_added), MONTH(date_added) ASC"))) {
